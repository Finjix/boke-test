"""Stateful orchestration for one v3 video localization job."""
...
class VideoLocalizationPipeline:
    ...
    def resume_failed(self, job_id: str, *, spec: JobSpec | None = None) -> Path:
        ...
        if failed_stage not in {stage.value for stage in PIPELINE_STAGES}:
            raise ValidationError("Checkpoint does not identify a resumable v3 stage")
        return self.run(
            effective_spec,
            job_id=job_id,
        ...
        if job_id and checkpoint_path.is_file():
            ...
            if (
                ...
                raise ValidationError(
                    "Job target settings do not match the existing v3 checkpoint; start a new job"
                )
    ...
    def _assert_checkpoint_compatible(self, value: Any, job_id: str) -> None:
        ...
        if not isinstance(artifacts, dict) or set(artifacts) - _V3_ARTIFACTS:
            raise ValidationError(
                f"Checkpoint for {job_id} contains unsupported artifact state; 需要新建任务 (start a new job)"
            )
        cache_key = value.get("cache_key")
        if not isinstance(cache_key, dict) or set(cache_key) != _V3_CACHE_KEY_FIELDS:
            raise ValidationError(
                f"Checkpoint for {job_id} has no valid v3 cache key; 需要新建任务 (start a new job)"
            )
        spec = value.get("spec")
        ...
        for field in ("task_ids", "request_ids"):
            ...
            if not isinstance(entries, dict) or set(entries) - _V3_TASK_KEYS:
                raise ValidationError(
    ...
    def _assert_cache_key_current(self, context: JobContext, spec: JobSpec) -> None:
        ...
        if context.cache_key != expected:
            raise ValidationError(
                "Existing checkpoint cache key does not match the current v3 configuration; "
        ...
        if spec.input_video.is_file():
            ...
            if requested_source["source_video_hash"] != context.cache_key["source_video_hash"]:
                raise ValidationError(
                    "Input video does not match the existing v3 checkpoint; 需要新建任务 (start a new job)"
                )
    ...
    def _generate_video(self, context: JobContext, state: dict[str, Any]) -> None:
        ...
        if "final_video" in state and state["final_video"].is_file():
            return
        ...
        refreshed_assets = [
            self._refresh_asset(context, asset) for asset in assets
        ]
        ...
        final_info_path = context.job_dir / "json/final_info.json"
        write_json(final_info_path, final_info.raw)
        self._set_artifact(context, "final_info", final_info_path)
        state["final_video"] = final_video
