...
class HistoryPanel(ttk.Frame):
    """List persisted jobs and expose only explicit H3 actions."""
    ...
    def _render_context(self, context: JobContext) -> None:
        ...
        if context.spec.reference_images:
            lines.extend(["", f"用户参考图：{len(context.spec.reference_images)} 张（只读）"])
    ...
    def _delete_selected(self) -> None:
        ...
        if entry.compatible:
            ...
            if context and any(
                ...
                messagebox.showwarning("无法删除", "仍有 H3 task 在云端运行，请先继续等待。")
                return
